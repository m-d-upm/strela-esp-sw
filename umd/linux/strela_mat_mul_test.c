...
void mat_mul_test()
{
    ...
    uint32_t *cgra_kernel = mat_mul_kernel_dot2;
#else
    ...
    for (int i = 0; i < MATRIX_DIM; i++) 
        for (int j = 0; j < MATRIX_DIM; j++)
#endif
            {
                ...
                cgra_ctrl.csrs.in3_offs = (i + MATRIX_DIM / 2) * MATRIX_DIM * sizeof(strela_data_t);
                cgra_ctrl.csrs.in3_count = MATRIX_DIM * sizeof(strela_data_t);
                cgra_ctrl.csrs.in3_stride = sizeof(strela_data_t);

                cgra_ctrl.csrs.in2_offs = MATRIX_SIZE  * sizeof(strela_data_t) + j * MATRIX_DIM * sizeof(strela_data_t);
                cgra_ctrl.csrs.in2_count = MATRIX_DIM * sizeof(strela_data_t);
                cgra_ctrl.csrs.in2_stride = sizeof(strela_data_t);

                cgra_ctrl.csrs.out2_offs = ((i + MATRIX_DIM / 2) * MATRIX_DIM + j) * sizeof(strela_data_t);
                cgra_ctrl.csrs.out2_count = sizeof(strela_data_t);
#else
                cgra_ctrl.csrs.in0_offs = i * MATRIX_DIM * sizeof(strela_data_t);
                cgra_ctrl.csrs.in0_count = MATRIX_DIM * sizeof(strela_data_t);
                cgra_ctrl.csrs.in0_stride = sizeof(strela_data_t);
