...
#define STRELA_OUT_BITS_STRIDE8_COUNT(count) ( count * 8U )
...
#define RELU_KRNL_BYTES (RELU_KRNL_SIZE * sizeof(unsigned))
...
unsigned relu_kernel[RELU_KRNL_SIZE] = {
...
static void init_buf(token_t *in, native_t *gold)
{
    ...
    // perform ReLu using CPU to obtain golden sample
    for(int i = 0; i < BUFF_SIZE; i++)
    {
        gold[i] = in[i] > 0 ? in[i] : 0;
    }
}
...
int main(int argc, char *argv[])
{
    ...
    *coh_reg_addr = coh;
    ...
    unsigned *cgra_kernel = relu_kernel;

    iowrite32(dev, STRELA_CONF_ADDR_A, (unsigned)cgra_kernel);
    iowrite32(dev, STRELA_CONF_SIZE_A, RELU_KRNL_BYTES);
